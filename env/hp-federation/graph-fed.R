...
eascG1 <- eascGA[which(eascGA$datacenter == "hp_phoenix") ,]

easc2 <- eascA[which(eascA$datacenter == "hp_boston") ,]
eascE2 <- eascEA[which(eascEA$datacenter == "hp_boston") ,]
...
ren <- renWatt(easc1$watts, 0 , 0, pv1$capacity, grid1$renPct)
renpct <- sum(ren) / (sum (easc1$watts) )* 100
lines (easc1$slot, sla$Sla, col="black", lty = 2)
totWatts <- (mean(easc1$watts) )*24
totRen <- mean(ren)*24
...
legend (list(x=50, y=1000), legend=leg2.txt)
...
ren <- renWatt(easc3$watts, eascG1$watts , 0, pv3$capacity, grid3$renPct)
renpct <- sum(ren) / (sum (easc3$watts) + sum(eascG1$watts))* 100
lines (easc1$slot, sla$Sla, col="black", lty = 2)
totWatts <- (mean(easc3$watts) + mean(eascG1$watts))*24
totRen <- mean(ren)*24
cat("Total watts = ", totWatts, " Total ren = ", totRen, "\n")
...
fill_area(easc1$slot, tot_fill, "brown", 200)
tot_fill <- tot_fill - eascG1$watts;
fill_area(easc1$slot, tot_fill, "orange", 200)
 tot_fill <- tot_fill - eascE2$watts;
fill_area(easc1$slot, tot_fill, "red", 200);
tot_fill <- tot_fill - easc3$watts;
fill_area(easc1$slot, tot_fill, "green", 200);
tot_fill <- tot_fill - easc2$watts;
fill_area(easc1$slot, tot_fill, "blue", 200);
...
lines (easc1$slot, sla$Sla/4, col="black", lwd=2, lty=2)
#lines (easc1$slot,grid$renPct, col="gray", lwd=2) 
lines (easc1$slot,(easc1$performance + easc2$performance + easc3$performance)/4, col="blue", lwd=3) 


leg.txt <- c("biz", "pv", "sla", "W1", "W2", "W3", "G1", "E2")
leg.col <- c("blue", "yellow", "black", "blue", "green", "red", "orange", "brown")
legend (list(x=85, y=1000), legend=leg.txt, col=leg.col, lty=1)
...
legend (list(x=0, y=1000), legend=leg2.txt)
